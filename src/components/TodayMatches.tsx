...
function GroupTime({ m }: { m: TodayMatch }) {
  return (
    <div className="flex flex-col gap-0.5">
      <span className="font-mono text-[13px] font-bold tabular-nums">{m.time}</span>
      <span className="text-[11px] font-semibold text-muted">Группа {m.group}</span>
    </div>
  );
}
...
function Row({ m }: { m: TodayMatch }) {
  ...
  return (
    <li className="px-3 py-3 transition-colors hover:bg-white/45 dark:hover:bg-white/[0.05] sm:px-4">
      ...
      <div className="hidden items-center gap-4 md:grid md:grid-cols-[78px_minmax(0,1fr)_140px_104px_96px_36px]">
        <GroupTime m={m} />
        <Teams m={m} />
        <div className="min-w-0">
          <div className="text-[10px] font-bold uppercase tracking-wide text-muted">Большинство</div>
          <div className="truncate text-[13px] font-bold">{fav}</div>
        </div>
        <div>
          <div className="text-[10px] font-bold uppercase tracking-wide text-muted">Частый счёт</div>
          <span className="mt-0.5 inline-block rounded-md bg-ink px-1.5 py-0.5 font-mono text-[12px] font-bold text-bg">
            {m.popularScore}
          </span>
        </div>
        <div className="text-right">
          <div className="text-[10px] font-bold uppercase tracking-wide text-muted">В игре</div>
        ...
        <Link
          href={`/groups#group-${m.group}`}
          aria-label="Детали группы"
  ...
  if (!matches.length) {
    return (
      <div className="glass p-6 text-center text-[14px] text-muted">
        Сегодня матчей нет — прогнозы зафиксированы, ждём следующий игровой день.
      </div>
    );
  }
    ...
    <div>
      <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <h2 className="font-display text-[22px] font-extrabold leading-tight sm:text-[26px]">
            {areToday ? "Матчи сегодня" : "Ближайшие матчи"}
          </h2>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white/55 px-4 py-3 text-[12px] leading-snug text-ink-soft ring-1 ring-black/5 dark:bg-white/8 dark:ring-white/12">
        <span className="inline-flex items-center gap-2">
          ...
          Очки распределяются по точности прогноза: точный счёт, исход, разница мячей. Сегодня в игре до {potentialTotal} очков.
        </span>
